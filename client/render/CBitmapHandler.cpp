...
SDL_Surface * BitmapHandler::loadH3PCX(ui8 * pcx, size_t size)
{
	...
	if (format==PCX8B)
	{
		...
		SDL_Palette * palette = SDL_CreateSurfacePalette(ret);
...
SDL_Surface * BitmapHandler::loadBitmapFromDir(const ImagePath & path)
{
	...
	try {
		...
		{ //loading via SDL_Image
			...
			if (palette)
			{
				// SDL3_image keeps images such as paletted png's indexed and stores their
				// transparency (png tRNS chunk) as alpha of the palette entries, while SDL2_image
				// used to expand those to RGBA. Do that expansion here, so that images with real
				// transparency stay on the same code path as before.
