...
{
	// SDL_GetRenderScale no longer reflects the logical presentation in SDL3, so
	// let the renderer do the conversion. Interface coordinates have to be brought
	// into the renderer's (upscaled) space first.
...
Point ScreenHandler::getRenderResolution() const
{
	...
	Point result;
	SDL_GetCurrentRenderOutputSize(mainRenderer, &result.x, &result.y);
