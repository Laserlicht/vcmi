...
static GlslPass *ParseConfigKeyPass(GlslShader *gs, const char *key, const char *match) {
  ...
  if ((uint8_t)(*key - '0') >= 10)
    return NULL;
  unsigned int pass = strtoul(key, &endp, 10);
  if (pass >= gs->n_pass || *endp != 0)
    return NULL;
  return gs->pass + pass + 1;
}

static uint8_t ParseScaleType(const char *s) {
...
static unsigned int ParseWrapMode(const char *s) {
...
void GlslShader_ReadShaderFile(GlslShader *gs, const char *filename, ByteArray *result) {
  ...
  while ((line = NextDelim(&data, '\n')) != NULL) {
    ...
    if (linelen >= 8 && memcmp(line, "#include", 8) == 0) {
      ...
      line[linelen] = '\n';
      ByteArray_AppendData(result, (uint8_t *)line, linelen + 1);
    }
  }
  free(data_org);
}

size_t LengthOfInitialComments(const uint8_t *data, size_t size) {
  const uint8_t *data_org = data, *data_end = data + size;
  while (data != data_end) {
    uint8_t c = *data++;
    if (c == ' ' || c == '\t' || c == '\r' || c == '\n')
      continue;
...
static bool GlslPass_Compile(GlslPass *p, unsigned int type, const uint8_t *data, size_t size, bool use_opengl_es) {
...
static void GlslTextureUniform_Read(unsigned int program, const char *prefix, int i, GlslTextureUniform *result) {
  char buf[40];
...
static void GlslShader_GetUniforms(GlslShader *gs) {
  ...
  for (GlslPass *p = gs->pass + 1, *p_end = p + gs->n_pass; p < p_end; p++, pass_idx++) {
    unsigned int program = p->gl_program;
...
typedef struct RenderCtx {
  ...
  unsigned int vaos[kMaxVaosInRenderCtx];
} RenderCtx;

static void RenderCtx_SetTexture(RenderCtx *ctx, int textureu, unsigned int texture_id) {
  if (textureu >= 0) {
...
static void RenderCtx_SetGlslTextureUniform(RenderCtx *ctx, GlslTextureUniform *u,
                                            int width, int height, unsigned int texture) {
...
void GlslShader_Render(GlslShader *gs, GlTextureWithSize *tex, int viewport_x, int viewport_y, int viewport_width, int viewport_height) {
  ...
  for (int pass = 1; pass <= gs->n_pass; pass++) {
    ...
    case GLSL_ABSOLUTE: p->width = (uint16_t)p->scale_x; break;
    case GLSL_SOURCE: p->width = (uint16_t)(p[-1].width * p->scale_x); break;
    case GLSL_VIEWPORT: p->width = (uint16_t)(viewport_width * p->scale_x); break;
    default: p->width = (uint16_t)(last_pass ? viewport_width : (p[-1].width * p->scale_x)); break;
    }

    switch (p->scale_type_y) {
    case GLSL_ABSOLUTE: p->height = (uint16_t)p->scale_y; break;
    case GLSL_SOURCE: p->height = (uint16_t)(p[-1].height * p->scale_y); break;
    case GLSL_VIEWPORT: p->height = (uint16_t)(viewport_height * p->scale_y); break;
    default: p->height = (uint16_t)(last_pass ? viewport_height : (p[-1].height * p->scale_y)); break;
    }
    ...
    unsigned int filter = p->filter ? p->filter : (last_pass && CFG_LINEAR_FILTERING) ? GL_LINEAR : GL_NEAREST;
