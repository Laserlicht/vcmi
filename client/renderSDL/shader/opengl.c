...
static uint8_t *g_screen_buffer;
...
static bool OpenGLRenderer_Init(SDL_Window *window) {
  ...
  if (CFG_SHADER)
    g_glsl_shader = GlslShader_CreateFromFile(CFG_SHADER, g_opengl_es);
...
static void OpenGLRenderer_BeginDraw(int width, int height, uint8_t **pixels, int *pitch) {
...
static void OpenGLRenderer_EndDraw() {
  ...
  if (g_glsl_shader == NULL) {
    ...
    int filter = CFG_LINEAR_FILTERING ? GL_LINEAR : GL_NEAREST;
