...
MemBlk FindIndexInMemblk(MemBlk data, size_t i) {
  ...
  size_t mx = *(uint16_t *)(data.ptr + end);
  if (mx < 8192) {
    if (i > mx || mx * 2 > end)
      return (MemBlk) { 0, 0 };
    left_off = ((i == 0) ? mx * 2 : mx * 2 + *(uint16_t *)(data.ptr + i * 2 - 2));
    right_off = (i == mx) ? end : mx * 2 + *(uint16_t *)(data.ptr + i * 2);
  } else {
    mx -= 8192;
    if (i > mx || mx * 4 > end)
      return (MemBlk) { 0, 0 };
    left_off = ((i == 0) ? mx * 4 : mx * 4 + *(uint32_t *)(data.ptr + i * 4 - 4));
    right_off = (i == mx) ? end : mx * 4 + *(uint32_t *)(data.ptr + i * 4);
  }
...
static uint64_t BpsDecodeInt(const uint8_t **src) {
...
uint8_t *ApplyBps(const uint8_t *src, size_t src_size_in,
  ...
  const uint8_t *bps_end = bps + bps_size - 12;

  if (memcmp(bps, "BPS1", 4))
    return NULL;
  if (crc32(src, src_size_in) != *(uint32_t *)(bps_end))
    return NULL;
  if (crc32(bps, bps_size - 4) != *(uint32_t *)(bps_end + 8))
    return NULL;

  bps += 4;
  uint32_t src_size = BpsDecodeInt(&bps);
  uint32_t dst_size = BpsDecodeInt(&bps);
  uint32_t meta_size = BpsDecodeInt(&bps);
  uint32_t outputOffset = 0;
  uint32_t sourceRelativeOffset = 0;
  uint32_t targetRelativeOffset = 0;
  ...
  *length_out = dst_size;
  ...
  if (!dst)
    return NULL;
  while (bps < bps_end) {
    uint32_t cmd = BpsDecodeInt(&bps);
    uint32_t length = (cmd >> 2) + 1;
    switch (cmd & 3) {
    case 0:
      while(length--) {
  ...
  if (dst_size != outputOffset)
    return NULL;
  if (crc32(dst, dst_size) != *(uint32_t *)(bps_end + 4))
    return NULL;
  return dst;
}