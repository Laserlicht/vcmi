...
void InputSourceMouse::handleEventMouseWheel(const SDL_MouseWheelEvent & wheel)
{
	...
	// where a single event carries only a fractional value.
	// We accumulate those and dispatch only whole steps to avoid losing scroll input.
	// Note: the values are already normalised for SDL_MOUSEWHEEL_FLIPPED (natural
	// scrolling), so no extra direction handling is needed here.
