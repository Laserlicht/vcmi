...
#ifdef VCMI_SWITCH
	auto path = archiveName / relativePath; // canonical() fails on libnx device paths
#else
...
{
#ifdef VCMI_SWITCH
	return  boost::filesystem::last_write_time(archiveName); // canonical() fails on libnx device paths
#else
