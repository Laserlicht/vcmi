...
/// When the previous frame reached the panel, so that the gap to the next one can be measured
EGLnsecs lastDisplayPresentTime = 0;
...
ReadResult reportFrame(EGLFrameId frameId)
{
	...
	if(reportPresentTime)
	{
		...
		// the gap between two displayed frames is the rate the panel really runs at, which
		// Android switches on its own - such a switch shows up here as a step
		if(lastDisplayPresentTime != 0)
			VCMI_PROFILE_PLOT_FLOAT("Frame: display interval (ms)", ms(lastDisplayPresentTime, values[4]));

		lastDisplayPresentTime = values[4];
	}
...
void reportCompositorInterval()
{
	...
	// an absent plot would otherwise look like a compositor that never changed its interval
	static bool reported = false;
	if(!reported)
	{
		reported = true;
		logGlobal->info("Compositor timing unavailable: eglGetCompositorTimingANDROID gave no interval");
	}
}
}
...
void FrameTimestamps::collect()
{
	...
	while(!pendingFrames.empty())
	{
		...
		// the gap to the next frame would span the one that was lost here
	...
	// the compositor changes this while running, so it is sampled per frame rather than once
	reportCompositorInterval();
}
